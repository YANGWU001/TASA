...
def load_dialogue(method, dataset, student_id, concept_text):
    """加载dialogue - 支持子目录结构（如FS方法）"""
    # 首先尝试直接路径
    dialogue_file = f'/mnt/localssd/bank/dialogue/{method}/{dataset}/{student_id}-{concept_text}.json'
    
    if not os.path.exists(dialogue_file):
        # 如果不存在，尝试在子目录中搜索（如dkt, akt等FS方法）
        base_dir = f'/mnt/localssd/bank/dialogue/{method}/{dataset}'
        if os.path.exists(base_dir):
            for subdir in os.listdir(base_dir):
                subdir_path = os.path.join(base_dir, subdir)
                if os.path.isdir(subdir_path):
                    candidate = os.path.join(subdir_path, f'{student_id}-{concept_text}.json')
                    if os.path.exists(candidate):
                        dialogue_file = candidate
                        break
            else:
                return None  # 子目录中也找不到
        else:
            return None  # base_dir不存在
...
1. **Teaching Clarity & Effectiveness** (60% weight)
   - Are explanations clear, accurate, and easy to understand?
   - Does it help the student learn the concept effectively?
   - Is the pedagogy sound (scaffolding, examples, feedback)?
   - Is it efficient and well-paced (not overly verbose)?
   - **IMPORTANT**: Clear, simple teaching often beats verbose personalization

2. **Personalization Value** (25% weight)
   - Does personalization **meaningfully improve** the learning experience?
   - Are student history references used effectively (not just mentioned)?
   - Does it adapt to student's level/gaps in a way that enhances understanding?
   - **KEY**: Only favor if personalization demonstrably helps; otherwise prefer simplicity

3. **Student Engagement & Appropriateness** (15% weight)
   - Is the content appropriate for this student's level?
   - Does it address the student's actual needs?

---

**Critical Decision Rules:**

⚠️ **You MUST choose "Tie" if (expect ~30-40% Ties):**
- Both dialogues successfully teach the concept (even if differently)
- Both reach similar learning outcomes by different paths
- One is more personalized BUT the other is clearer (trade-offs cancel out)
- The quality difference is small or marginal
- Both have reasonable pedagogy and appropriate content
- The personalized dialogue adds complexity without clear learning benefit
- **DEFAULT to Tie unless there's a MAJOR quality difference**
- **When in doubt, choose Tie**

✅ **Choose a winner ONLY if (should be ~60-70%, aim for balance):**
- One has a **SUBSTANTIAL** advantage in teaching effectiveness
- The quality difference is **obvious and undeniable**
- One dialogue would clearly produce better learning for THIS student
- **The bar for choosing a winner is HIGH**

🎯 **Dialogue A (personalized) should win ONLY if (aim ~25-30% wins):**
- Personalization provides **major, demonstrable** learning improvements
- It's significantly more effective at addressing student's specific needs
- The baseline is clearly generic/inadequate for this student
- **Personalization makes a BIG difference, not just a small one**

🎯 **Dialogue B (baseline) should win ONLY if (aim ~25-30% wins):**
- It's **much clearer** and more efficient at teaching
- The personalized dialogue is confusing, overly complex, or counterproductive
- Simplicity clearly produces better learning than verbose personalization
- **The baseline is clearly superior in teaching this concept**

⚖️ **Key Principle - Favor Ties:**
- If both dialogues would help the student learn → Tie
- If trade-offs balance out → Tie
- If the difference is subtle → Tie
- **Only choose a winner when the advantage is substantial and clear**
...
def batch_judge(target_method, dataset='assist2017', max_workers=20, baseline_method=None):
    """批量评估一个target method vs baseline"""
    
    # 确定backbone
    target_backbone = get_backbone(target_method)
    
    # 确定baseline method（如果没有指定）
