#!/usr/bin/env python3
"""
生成 overall.json v2 - 改进版
1. 为每种方法独立计算level（基于各自的FS分布）
2. 基于实际delta_t中位数自动计算合理的tau
"""
...
def calculate_method_levels(method_data, method_name):
    """为一个方法计算level"""
    fs_values = []
    
    for uid, concepts in method_data.items():
        for concept_text, concept_data in concepts.items():
            if 'fs' in concept_data:
                fs_values.append(concept_data['fs'])
    ...
    print(f"    {method_name:10} FS分布: min={min(fs_values):.4f}, q33={q33:.4f}, median={np.median(fs_values):.4f}, q67={q67:.4f}, max={max(fs_values):.4f}")
    
    # 为每个数据点分配level
    level_map = {}
    for uid, concepts in method_data.items():
        if uid not in level_map:
            level_map[uid] = {}
        ...
        for concept_text, concept_data in concepts.items():
            if 'fs' in concept_data:
                ...
                level_map[uid][concept_text] = level
...
def generate_overall(dataset):
    ...
    for method in methods:
        if method in method_data:
            level_map = calculate_method_levels(method_data[method], method)
            method_levels[method] = level_map
            ...
            for method in methods:
                ...
                if uid in method_data[method] and concept_text in method_data[method][uid]:
                    ...
                    if method in method_levels and uid in method_levels[method] and concept_text in method_levels[method][uid]:
                        level = method_levels[method][uid][concept_text]
