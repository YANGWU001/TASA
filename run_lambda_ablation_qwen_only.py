#!/usr/bin/env python3
"""
Lambda Ablation - Qwen Backbone Only
独立运行Qwen backbone的lambda ablation，不影响Llama版本
"""
...
def get_tutor_qwen(lambda_value):
    """获取Qwen版本的Tutor实例"""
    tutor_key = f'tutor_lambda{lambda_value}_qwen'
    if not hasattr(thread_local, tutor_key):
        with model_init_lock:
            safe_print(f"   [Thread-{threading.current_thread().ident}] 初始化TASA Tutor (Lambda={lambda_value}, Qwen)...")
...
Provide explanation and next question."""
                ...
                def save_dialogue(self, dialogue, student_id, concept_text, dataset):
                    ...
                    save_dir = f'/mnt/localssd/bank/dialogue/TASA-lambda{self.lambda_value}-qwen/{dataset}/{FORGETTING_SCORE_METHOD}'
            ...
            tutor = TASATutorLambdaQwen(lambda_value)
...
def process_single_student(student_id, dataset, lambda_value):
    ...
    try:
        safe_print(f"\n{'='*80}")
        safe_print(f"🎓 开始处理学生 {student_id} (Lambda={lambda_value}, Qwen)")
        safe_print(f"{'='*80}")
        ...
        from tasa_config import FORGETTING_SCORE_METHOD
        dialogue_file = f'/mnt/localssd/bank/dialogue/TASA-lambda{lambda_value}-qwen/{dataset}/{FORGETTING_SCORE_METHOD}/{student_id}-{concept_text}.json'
        
        if not os.path.exists(dialogue_file):
            safe_print(f"   📚 正在生成dialogue (Lambda={lambda_value}, Qwen)...")
            student_prompt = build_student_system_prompt(session)
            
            tutor = get_tutor_qwen(lambda_value)
...
def batch_evaluate(lambda_value, dataset='assist2017', max_workers=10):
    ...
    from tasa_config import FORGETTING_SCORE_METHOD
    result_dir = f'/mnt/localssd/bank/evaluation_results/TASA-lambda{lambda_value}-qwen/{dataset}/{FORGETTING_SCORE_METHOD}'
...
def main():
    ...
    print("╔══════════════════════════════════════════════════════════════════════════════╗")
    print("║              🔬 Lambda Ablation - Qwen Backbone Only                        ║")
    print("╚══════════════════════════════════════════════════════════════════════════════╝")
    print(f"\n📊 配置:")
    print(f"  • Lambda values: {LAMBDA_VALUES}")
    print(f"  • Backbone: Qwen3-4B")
    ...
    # 设置环境 - 使用Qwen config
    os.environ['TASA_CONFIG'] = 'tasa_config_qwen'
    ...
    for lambda_value in LAMBDA_VALUES:
        ...
        if result:
            qwen_results[str(lambda_value)] = result['strategy_max']['avg_learning_gain']
    
    total_elapsed = time.time() - start_time
    
    print(f"\n╔══════════════════════════════════════════════════════════════════════════════╗")
    print(f"║                   📊 Qwen Lambda Ablation完成                               ║")
    print(f"╚══════════════════════════════════════════════════════════════════════════════╝\n")
    
    print("结果 (Lambda → Learning Gain):\n")
    for lv in LAMBDA_VALUES:
        if str(lv) in qwen_results:
            lg = qwen_results[str(lv)] * 100
            print(f"  λ={lv}: {lg:.1f}%")
        else:
            print(f"  λ={lv}: -")
    ...
    results_file = '/mnt/localssd/logs/lambda_ablation_qwen_results.json'
